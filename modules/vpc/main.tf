locals {
  ...
  # Use `local.vpc_id` para dar uma dica ao Terraform de que as sub-redes devem ser excluídas antes que os blocos CIDR secundários possam ser liberados!
  vpc_id = element(
    concat(
...
#################
# Private routes
# Existem tantas tabelas de roteamento quanto o número de NAT Gateways
#################
resource "aws_route_table" "private" {
  ...
  lifecycle {
    // Ao conectar gateways VPN, é comum definir recursos aws_vpn_gateway_route_propagation que
    // manipulam os atributos da tabela de roteamento (normalmente para as sub-redes privadas)
...
#####################################################
# intra subnets - subnet privada sem nat gateway
#####################################################
...
# NAT Gateway
##############
// Solução alternativa para interpolação da condicional não causar erro
// Fonte: https://github.com/hashicorp/terraform/issues/11566#issuecomment-289417805
// A expressão lógica seria nat_gateway_ips = var.reuse_nat_ips? var.external_nat_ip_ids: aws_eip.nat. *. id
// mas quando a contagem de aws_eip.nat. *. id for zero, isso geraria um erro de recurso não encontrado em aws_eip.nat. *. id.
...
#######################
# VPC Endpoint para ECS Telemetry
#######################
...
#######################
# VPC Endpoint para CloudWatch Monitoring
#######################
...
#######################
# VPC Endpoint para CloudWatch Events
#######################
...
#######################
# VPC Endpoint para Elastic Load Balancing
#######################
